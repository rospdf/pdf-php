...
class RPDI extends Cpdf_Extension {
		...
		if(isset($entryValue) && is_array($entryValue)) {
			...
			foreach ($entryValue as $key => $value) {
				if(is_array($value)){
					...
					} else if($t['type'] == self::ENTRYTYPE_ARRAY){
	...
	private function readXRefAndTrailer($xrefpos, $isLinearized = false){
        ...
        } else if(count($this->objectRef) > 0) {
    ...
    private function parseEntry($entry, &$offset = 0){
        ...
        while($i < $length) {
            if($token == self::TOKEN_NONE && substr($entry, $i, 2) == '<<' ) {
                ...
                $i++;
            } else if($token == self::TOKEN_NONE && $bracketOpened <= 0 && substr($entry, $i, 2) == '>>') {
                $level--;
            ...
            } else if($token == self::TOKEN_NAME && substr($entry, $i, 2) == '<<') {
                $o = 0;
                $fields[$name] = $this->parseEntry(substr($entry, $i), $o);
                $i += $o;
                $token = self::TOKEN_NONE;
            } else if(($token == self::TOKEN_NAME || $token == self::TOKEN_BRACKET) && $entry[$i] == '[') {
                $bracketOpened++;
                $value.= $entry[$i];
                $token = self::TOKEN_BRACKET;
            } else if($token == self::TOKEN_BRACKET && $bracketOpened > 0 && $entry[$i] == ']') {
                $bracketOpened--;
                if($bracketOpened <= 0) {
                    $bracketOpened = 0;
                    $value .= $entry[$i];
            ...
            } else if($token == self::TOKEN_BRACKET) {
                $value .= $entry[$i];
                ...
                $token = self::TOKEN_NONE;
                $i--;
            } else if($token == self::TOKEN_NONE && $bracketOpened <= 0 && preg_match("/^\/([A-Z0-9]+)/i", substr($entry, $i), $regs)) {
                $name = $regs[1];
                $token = self::TOKEN_NAME;
                $i += strlen($regs[0]) - 1;
            } else if($token == self::TOKEN_NAME && $bracketOpened <= 0 && preg_match("/^([0-9]+ 0 R|[0-9]+|true|false|\/[A-Z0-9]+)/i", substr($entry, $i), $regs)) {
                $value = $regs[1];
                $token = self::TOKEN_VALUE;
	...
	private function parseType($subject){
		...
		} else if(preg_match("/^([0-9]+.[0-9]+)/", $subject, $regs)){
			$parsed['value'] = floatval($regs[1]);
			$parsed['type'] = self::ENTRYTYPE_FLOAT;
		} else if(preg_match("/^([0-9]+)/", $subject, $regs)) {
			$parsed['value'] = (int)$regs[1];
			$parsed['type'] = self::ENTRYTYPE_INT;
		} else if(preg_match("/^(true|false)/i", $subject, $regs)){
		...
		} else if(preg_match("/^\/(\w+)/i", $subject, $regs)){
			$parsed['value'] = $regs[1];
			$parsed['type'] = self::ENTRYTYPE_LABEL;
		} else if(preg_match("/^\[(.*?)\]/i", $subject, $regs)){
